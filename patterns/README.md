...
| `feedback.ts` | Self-contained feedback dialog: footer trigger, bug/idea toggle, optional email, focus trap, honeypot + dwell anti-spam, scoped `fbw-` styles. GENERATED from `gh-feedback/widget/feedback.ts` — never edit the copy. | **Every game.** Call `mountFeedback()` once after the footer exists. Because `body.playing` hides `.site-footer`, ALSO wire `openFeedback()` into the results screen — otherwise the trigger is invisible for the whole round, exactly when a player hits the bug they want to report. |
